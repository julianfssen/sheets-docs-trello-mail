# EZDocsSheetsDriveTrello
# By Julian Foo julianfssen@gmail.com | Al Sweigart al@inventwithpython.com
...
def init(
    ...
    docsTokenFile="token-docs.pickle",
    _raiseException=True,
):
    global SHEETS_SERVICE, DRIVE_SERVICE, DOCS_SERVICE,  IS_INITIALIZED
    ...
    try:
        ...
        if os.path.exists(docsTokenFile):
            with open(docsTokenFile, 'rb') as token:
                creds = pickle.load(token)
        ...
        if not creds or not creds.valid:
            ...
            with open(docsTokenFile, 'wb') as token:
                pickle.dump(creds, token)
